...
import '../../blocs/job_manage/job_manage_bloc.dart';
import '../header_page.dart';

class JobManageView extends StatefulWidget {
  const JobManageView({super.key, required this.params});

  final Map<String, String> params;

  @override
  State<JobManageView> createState() => _JobManageViewState();
}

class _JobManageViewState extends State<JobManageView> {
  late JobManageBloc _bloc;
  ...
  @override
  void didUpdateWidget(JobManageView oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
      ...
      child: BlocBuilder<JobManageBloc, JobManageState>(
                  ...
                  child: Column(
                    children: [
                      ...
                      for(int i = 0; i < jobs.length; i++)
                        bulidRow(state, jobs[i], state.isCheckeds, i),
                      Pagination(
                        path: '/job_manage',
            ...
            child: ElevatedButton(
              ...
              onPressed: () {
                if(state.isCheckeds.any((element) => element))
                  ...
                  WidgetsBinding.instance.addPostFrameCallback((_) {
                    showTopRightSnackBar(context, "Vui lòng chọn bài đăng để xóa!", NotifyType.warning);
                  });
              },
  ...
  Widget headerTable(List<bool> isCheckeds) {
    return Row(
      children: [
        ...
        Expanded(
          flex: 10,
          child: Text("Tiêu đề", style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600),),
        ),
        Expanded(
          flex: 5,
  ...
  Widget bulidRow(LoadSuccess state, Job job, List<bool> isCheckeds, int index) {
          ...
          content:  Container(
            ...
            child: SingleChildScrollView(
              child: const ListBody(
                children: <Widget>[
                  Text("Bạn có muốn xóa các bài đăng mà bạn đã chọn không?",
                    style: TextStyle(fontSize: 24),),
                  Text("Sau khi xác nhận, các bài đăng sẽ bị xóa!",
